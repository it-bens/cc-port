...
`archive`'s equivalent). The final surface in the list commits every open SQL
...
of where the previous attempt was killed. Both properties (crash rollback,
re-run idempotence) are covered by tests in each adapter package; this prose
