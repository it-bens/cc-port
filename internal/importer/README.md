...
- `IsImplicitKey(key string) bool`: predicate exposing the implicit-key set without leaking the constants. Covers `{{PROJECT_PATH}}` (supplied from the import target argument), `{{HOME}}` (supplied via `resolveHomeAnchor` from `os.UserHomeDir()` on the import machine), and `{{PROJECT_DIR}}` (supplied from `claude.Home.ProjectDir(targetPath)` on the import machine). Callers refuse user-supplied resolutions for these keys and treat them as already-resolved when computing unresolved sets.
...
- Implicit anchors: `{{PROJECT_PATH}}` is supplied by `importer.Run` from the import target argument, `{{HOME}}` is supplied by the cmd layer from the recipient machine's `os.UserHomeDir()` via `resolveHomeAnchor`, and `{{PROJECT_DIR}}` is supplied by `withImplicitAnchors` from `claude.Home.ProjectDir(TargetPath)` (the recipient's own encoded storage directory) so an imported body's encoded-dir reference points at the local dir rather than the sender's. `IsImplicitKey` reports all three. All three are treated as resolved even when absent from the caller's resolution map, and user-supplied resolutions for any of them are refused at the cmd layer.
...
- **Undeclared `{{UPPER_SNAKE}}` tokens in bodies.** All `{{X}}`-shaped substrings the manifest does not declare are treated as ordinary content. The export pipeline emits exactly `{{PROJECT_PATH}}`, `{{HOME}}`, and `{{PROJECT_DIR}}`; any other `{{KEY}}` in a body is chat content, mustache/handlebars syntax, or sample code. Tamper detection is not the importer's responsibility.
