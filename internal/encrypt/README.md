...
Streaming wrappers around `filippo.io/age` for cc-port archives plus self-skipping pipeline stage types. Symmetric (passphrase) mode only; key-recipient mode is intentionally out of scope. Higher-level callers (`cmd/cc-port`, `internal/sync`) consume this package; it does not import them.
...
Asymmetric (X25519) recipients. Adding them would change the public shape; do it as a separate spec, not as a flag.
...
Distinguishing wrong-passphrase from tampered-file at the API surface. age conflates the cases on purpose; this package follows.
...
Bytes that match the age magic prefix but are not actual age archives. The decrypt drain fails with `ErrPassphrase`; callers see the same unified error.

### Tempfile lifecycle

#### Handled

`ReaderStage.Open` materializes plaintext into `os.TempDir()` with mode `0600`. The returned `Source.Close` removes the tempfile and chains to upstream. Idempotent; second call returns nil.
...
SIGKILL between `ReaderStage.Open` and `Source.Close` leaves a tempfile in `os.TempDir()`. The OS tempdir cleanup handles eventually. Decrypt size is bounded by the ciphertext byte count (age plaintext-to-ciphertext ratio is roughly 1:1); no separate cap is enforced because there is no decompression-bomb amplification path.
