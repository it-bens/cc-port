...
- Refusal messages are surface-specific — each entry point names the non-interactive alternative that applies to its flow (category flags, manifest flow, `--resolution`); only `ConfirmApply` has no alternative and says so explicitly (README §Interactive prompts require a TTY).
