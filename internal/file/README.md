...
- `Source{Path}`: ReaderStage. Opens Path for reading; returns the `*os.File` as `pipeline.Source.ReaderAt`.
- `Sink{Path}`: WriterStage. Creates Path with mode 0600, `O_CREATE|O_WRONLY|O_TRUNC`. Existing files are truncated.
