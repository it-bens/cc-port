...
- The `.` lookahead must stay two bytes wide; a one-byte check breaks either extension suppression (`/a/foo.v2`) or sentence-terminating prose (`see /a/foo.`) (README §Boundary rules).
- `FindPlaceholderTokens` is the tamper-defense scan — its grammar is intentionally narrow to `{{UPPER_SNAKE}}`; widening it produces false positives on legitimate `{{…}}` content in transcripts (README §Boundary rules §Residual risk).
- `SafeRenamePromoter.Rollback` must restore pre-promote bytes for every renamed destination; don't skip entries on partial failure (README §Boundary rules).
