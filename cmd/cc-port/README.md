...
`importcmd_test.go` in this package tests `parseResolutionFlags` flag parsing and validation. `category_selection_test.go` pins the `--from-manifest` exclusivity rule across every per-category flag. Most behavioral tests live in the owning `internal/*` packages. Push and pull dispatch tests (`openPriorRead`, `openArchiveSource`) live alongside the cmd helpers because the dispatch is owned here. `integration_test.go` at the repo root runs full CLI end-to-end against a fixture `~/.claude`.
