...
- `code.primitives` =
  ...
  | Rewrite a path stored in a SQLite column | Hand-written `UPDATE` with `replace()`/`LIKE`; byte-editing the `.sqlite` file | `sqlrewrite.Open`, then `DB.Begin` and `DB.RewriteTextColumn` / `DB.UpdateColumnsByKey` / `DB.UpdateColumnsByRowID` inside the `*Tx`; count with `sqlrewrite.CountTextColumnRO` | `LIKE` wildcards/case-folds and SQL `replace()` corrupts prefix-sharing paths; wrapper matches paths in Go with the boundary-aware `rewrite` helpers behind an `instr` prefilter, writes each changed row back by key (keyed updates can require `COLLATE BINARY` expected values), and carries WAL checkpoint discipline and column validation. |
