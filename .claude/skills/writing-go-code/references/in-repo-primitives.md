...
| Read or rewrite a zstd-compressed session log | `zstd.NewReader` plus a hand-rolled scanner | `codex.TranscodeLines` with `TranscodeCaps` | A decompression bomb expands past any on-disk size check. The transcoder caps the whole decompressed stream and each line; the two caps guard different bomb shapes and are both mandatory. |
