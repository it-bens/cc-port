...
- For `move --apply`, preflight every selected tool with witness-first `lock.Acquire` in registry order, hold all flocks through apply, and release in reverse order; wrap `import --apply` in nested `lock.WithLock` across selected tools. A plan path takes no lock. (internal/lock/README.md §Concurrency guard)
